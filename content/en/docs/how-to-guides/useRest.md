...
1. JWT has to be generated by either a dedicated API user (has only `api` role) or by an _administrator_ with both `admin` and `api` [roles]({{< ref "roles" >}} "ClusterCockpit Roles").
2. JWTs have a limited lifetime, i.e. will become invalid after a configurable amount of time (see `auth.jwt.max-age` [config option]({{< ref "ccb-configuration" >}} "ClusterCockpit Configuration")).
3. Administrator endpoints are additionally subjected to a configurable IP whitelist (see `api-allowed-ips` [config option]({{< ref "ccb-configuration" >}} "ClusterCockpit Configuration")). Per default there is no restriction on IPs that can access the endpoints.
...
1. JWT has to be generated by either a dedicated API user (Has only `api` role) or an _User_ with additional `api` [role]({{< ref "roles" >}} "ClusterCockpit Roles").
