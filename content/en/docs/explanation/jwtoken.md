...
[here]({{< ref "ccb-configuration" >}} "Job Metadata Schema Reference").
...
1. `config.json`: Insert a name for the cookie (set by the external service)
...
1. Make sure your external service includes the same issuer (`iss`) in its JWTs.
   Example JWT payload:
